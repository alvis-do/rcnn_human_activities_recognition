    ...
    with g.name_scope("YOLO"):
        def conv(n, in_name, out_channels, kernel_size, stride, nonlin="relu", keep_prob=1, batchnorm=1):
            ...
            with g.name_scope("conv_{}".format(n)):
                kernel = tf.Variable(tf.random_uniform(shape = [kernel_size, kernel_size, in_channels, out_channels], maxval=0.001), dtype = tf.float32, name = "kernel")
                ...
                #kernel = tf.Print(kernel, [kernel], summarize=100, message="conv_{}".format(n))
                '''
                conv
        ...
        def yolo(n, in_name, anchor, thresh=0.5):#in tensor has shape (batch_size, height, width, 255)
            ...
            split = tf.split(in_tensor, 1, axis = 3)
            new_split = []
        ...
        conv(15, "YOLO/conv_14/out:0", out_depth, 1, 1, nonlin = "linear", batchnorm=0)
        #16

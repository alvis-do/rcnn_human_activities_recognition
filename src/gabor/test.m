%load('/Users/thoithanh/Desktop/ucf_gabor_model.mat','Mdl','k_feature');
%load('/Users/thoithanh/Desktop/BC/model_86.mat','Mdl','k_feature');
index_of_feature_retained = k_feature(1,:);
...
%     C = strsplit(tline,'/');
%     y_true = C(7);
%     test_feature = vid2feature_fn(tline);
%     if(size(test_feature,2)<150)
%         break
%     end
%     test_feature = test_feature(:,index_of_feature_retained);
%     [label, score] = predict(Mdl, test_feature);
...
position_delete_points = find( new_points(:,1)<temp_extrema(2) | new_points(:,1)>(temp_extrema(2)+temp_extrema(4)) | new_points(:,2) < temp_extrema(1) | new_points(:,2)>(temp_extrema(1)+temp_extrema(3)));    position_delete_points = position_delete_points.';
%position_delete_points = find( new_points(:,2)<temp_extrema(2) | new_points(:,2)>(temp_extrema(2)+temp_extrema(4)));    
...
%aa = gabor2_fn(im,size,sigma,theta,lambda,psi,gamma);
